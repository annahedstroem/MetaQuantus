...
def plot_category_convergence(means: np.array, stds: np.array, save: bool,
    ...
    ax.set_ylabel("Spearman Rank Correlation", fontsize=16)
    ax.set_xticks(
...
def plot_hp_bar(df_result: pd.DataFrame,
    ...
    barlist1=ax.bar(settings, iec_nr, alpha=0.65, color='#b1d1fc', label="IEC_NR", edgecolor="black") ##aaa662
    barlist2=ax.bar(settings, iac_nr, alpha=0.65,  bottom=iec_nr,  color='gray', label="IAC_NR",  hatch="", edgecolor="black") ##719f91
    barlist3=ax.bar(settings, iec_ar, alpha=0.65, bottom=(iac_nr+iec_nr),  color='#5a86ad',  label="IEC_AR", edgecolor="black") ##c87f89
    barlist4=ax.bar(settings, iac_ar, alpha=0.65, bottom=(iec_ar+iac_nr+iec_nr), yerr=mc_scores_std, color='#c5c9c7', label="IAC_AR", hatch="", edgecolor="black") #738595#afa88b
    ...
    ax.set_ylabel("Relative Scoring Criteria", fontsize=16)
